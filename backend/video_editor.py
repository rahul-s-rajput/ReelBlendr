        ...
        while retry_count < max_retries:
            try:
                ...
                if response and response.text:
                    break
                print(f"Empty response from Gemini, attempt {retry_count + 1}/{max_retries}")
            except Exception as e:
                print(f"Error calling Gemini API (attempt {retry_count + 1}/{max_retries}): {e}")
            retry_count += 1
                ...
                for audio_idx, audio_seg in enumerate(audio_segments):
                    ...
                    if best_segment:
                        ...
                        final_segment = {
                            ...
                            'end_time': seg['start_time'] + audio_duration + 0.2,
